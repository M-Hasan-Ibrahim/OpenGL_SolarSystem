...
//creates, links, and activates the main shader program
void initGPUprogram() {
...
//initializes CPU-side geometry arrays
...
void initCPUgeometry() {
  ...
  g_mesh.buildSphere(stacks, slices);

}

//OLD GPUGEOMETRY
//uploads CPU geometry to GPU buffers and configures the VAO
...
//   glBufferData(GL_ARRAY_BUFFER, sizeof(float)*g_vertexColors.size(), g_vertexColors.data(), GL_STATIC_DRAW);
...
//master initializer, calls all initializers
void init() {
  initGLFW();
  initOpenGL();
  initCPUgeometry();
...
//renders one frame: clear buffers, uploads view/proj uniforms, binds VAO, draws indexed triangles
