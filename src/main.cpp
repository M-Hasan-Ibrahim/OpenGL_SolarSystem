...
void loadShader(GLuint program, GLenum type, const std::string &shaderFilename) {
  ...
  std::string shaderSourceString = file2String(shaderFilename); // Loads the shader source from a file to a C++ string
   ...
   if (shaderSourceString.empty()) {
        std::cerr << "shader file empty or not found: " << shaderFilename << std::endl;
        return;
    }
...
void initGPUprogram() {
  ...
  loadShader(g_program, GL_VERTEX_SHADER, "../src/vertexShader.glsl");
  loadShader(g_program, GL_FRAGMENT_SHADER, "../src/fragmentShader.glsl");
